...
| Reuse drawing styles in [TrendChart](../Packages/StrandDesign/Sources/StrandDesign/TrendChart.swift) and [OverviewHRChart](../Packages/StrandDesign/Sources/StrandDesign/OverviewHRChart.swift), and find the nearest full-resolution reading by date. | The area gradient is built once per chart update rather than once per mark. Touch and pointer readouts use a binary search while keeping the original readings and earlier-point tie rule. Device impact remains to be measured. |
