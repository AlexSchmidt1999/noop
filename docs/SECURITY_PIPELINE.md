...
Full MobSF IPA analysis runs on manually dispatched candidate builds after `security-gate`; it is skipped for ordinary PRs because of container size and run time. The local Personal Team installer requires this job to pass at approval time, then reuses the pinned artifact for subsequent renewals. The MobSF job has no Apple secrets. The optional paid `trusted-signing` job still requires its separate Environment review.
